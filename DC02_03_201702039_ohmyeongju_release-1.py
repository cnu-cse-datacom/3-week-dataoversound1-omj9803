...
def match(freq1, freq2):        # freq2 = handshake end hz
    return abs(freq1 - freq2) < 20          # freq 1,2 's differencd 20 iha
...
def extract_packet(freqs):  # freqs = packet
    freqs = freqs[::2]
...
def listen_linux(frame_rate=44100, interval=0.1):
    ...
    while True:
        ...
        if in_packet and match(temp_dom, HANDSHAKE_END_HZ) and isEnd is True: # real end sign
            ...
            if count_1540 == 2:
                ...
                temp_chunk = np.fromstring(data2, dtype=np.int16)
                temp_dom = dominant(frame_rate, temp_chunk)

                qq = int(dom) - int(temp_dom)
                if qq >= 0:
                    isEnd = True

                else:
                    count_1540 = 0
                    packet.append(temp_dom)
